...
// Accordion
const accordionBtns = document.querySelectorAll("[data-gh-toggle]");

accordionBtns.forEach((btn) => {
	btn.addEventListener("click", () => {
		const listItem = btn.closest("li");
		const contentElement = document.querySelector(btn.getAttribute("data-gh-target"));
		btn.classList.toggle("active");
		getComputedStyle(contentElement, null).display === "block" ? contentElement.style.display = "none" : contentElement.style.display = "block";
	});
});
